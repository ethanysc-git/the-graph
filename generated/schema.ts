...
export class ItemActive extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ItemActive entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type ItemActive must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("ItemActive", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): ItemActive | null {
    return changetype<ItemActive | null>(
      store.get_in_block("ItemActive", id.toHexString()),
    );
  }

  static load(id: Bytes): ItemActive | null {
    return changetype<ItemActive | null>(
      store.get("ItemActive", id.toHexString()),
    );
  }
